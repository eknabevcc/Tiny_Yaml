...
namespace tests {
    ...
    bool test_1_variables(){
        ...
        try{
            ...
            total++; passed += assert(yamlFile["parent-label"].getData<std::string>(), std::string(""));
            total++; passed += assert(yamlFile["parent-label"]["window"].getData<std::string>(), std::string("60 60 800 400"));
            total++; passed += assert(yamlFile["parent-label"]["child_with_dash"].getData<std::string>(), std::string("this-is also 101_ok"));
            total++; passed += assert(yamlFile["same_key_twice"].getData<std::string>(), std::string("\"this is second\""));
            total++; passed += assert(yamlFile["missing_value"].getData<std::string>(), std::string(""));
